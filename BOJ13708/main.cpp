...
Circle calculateCircleFromPoints(std::vector<Point>& reservedPoints)
{
    ...
    else if (size == 2)
    {
        ...
        circle.diameter = distance(reservedPoints[0], reservedPoints[1]);
    }
    else if (size == 3)
    {
        ...
        circle.center = {ux, uy};
        circle.diameter = distance(circle.center, A) * 2.0f;
    }
...
{
    return distance(circle.center, point) * 2.0f <= circle.diameter;
}
...
int main()
{
    ...
    std::cout << std::fixed << std::setprecision(2) << circle.diameter << "\n";
